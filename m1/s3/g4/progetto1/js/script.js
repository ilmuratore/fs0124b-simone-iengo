// funzione che genera l'html con i numeri del tabellone 

const generateMainBoard = function () {
  const board = document.querySelector('.main-board')
  for (let i = 0; i < 90; i++) {
    board.innerHTML += `<div class='cell-number'>${i + 1}</div>`
  }
}
...
const generateRandNumber = function (range) {
  ...
  const cells = document.querySelectorAll('.main-board .cell-number')
  cells[random - 1].classList.add('checked')

  const userCells = document.querySelectorAll('.user-board .cell-number')
...
const generateUserBoards = function () {
  ...
  if (parseInt(usersNumber) > 0) {
    for (let i = 0; i < parseInt(usersNumber); i++) {
      ...
      for (let i = 0; i < 24; i++) {
        const random = getRandomNum(range)
        board.innerHTML += `<div class='cell-number'>${random}</div>`
      }
      container.appendChild(board)
    }
  }
}
// fa partire le funzioni js ad avvio della pagina 
